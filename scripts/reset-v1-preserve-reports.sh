...
if [ "${CONFIRM_MEDIAFORGE_RESET:-}" != "YES" ]; then
  echo "Refusing to reset MediaForge."
  ...
  echo ""
  echo "Run with:"
  echo "  CONFIRM_MEDIAFORGE_RESET=YES sh scripts/reset-v1-preserve-reports.sh"
  exit 1
fi

PROJECT_NAME="${COMPOSE_PROJECT_NAME:-mediaforge}"
DB_VOLUME="${PROJECT_NAME}_backend-data"

echo "Stopping MediaForge containers..."
docker compose down
...
echo "Starting MediaForge with V1 seed data..."
