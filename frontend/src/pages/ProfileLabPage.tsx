...
const emptyTrackDraft: TrackProfile = {
  ...
  fontAttachmentExportPolicy: 'none',
  chapterPolicy: 'keep',
  notes: '',
};
...
export function ProfileLabPage() {
  ...
  function selectVideoProfile(profileId: number) {
    ...
    const workerConfig = isHardwareEncoderOption(encoder) && qualityPreset && qualityPreset !== 'custom'
      ? applySharedHardwareQualityPreset(draft.workerConfig ?? {}, encoder, qualityPreset)
      : draft.workerConfig;
    const selectedDraft = {
      ...draft,
  ...
  function selectHardwareQualityPreset(preset: string) {
    const encoder = videoWorkerValue(videoDraft, 'videoEncoder', 'auto');
    const requested = synchronizeLabAuthoritativeContract({
      ...videoDraft,
  ...
  currentFidelityInspection?.ffmpegArgs?.length
    ? [
        'ffmpeg',
        ...currentFidelityInspection.ffmpegArgs.map((argument) =>
          shellQuote(argument),
        ),
      ].join(' ')
    : '';
    ...
    <>
      ...
      <Box sx={{ px: { xs: 2, md: 4 }, pb: 4 }}>
            ...
            <Stack spacing={1.25}>
              ...
              <Collapse in={terminalCommandOpen}>
                <Stack spacing={1}>
                  ...
                  />
                  {effectivePreviewCommand && (
                    <section className="profile-lab-command">
                      <div className="profile-lab-command__header">
                        <strong>Effective preview command</strong>
                        <span>
                          Generated by backend
                          {currentFidelityInspection?.effectiveQSVRateControl
                            ? currentFidelityInspection.requestedQSVRateControl &&
                              currentFidelityInspection.requestedQSVRateControl !==
                                currentFidelityInspection.effectiveQSVRateControl
                              ? ` · QSV ${currentFidelityInspection.requestedQSVRateControl.toUpperCase()} → ${currentFidelityInspection.effectiveQSVRateControl.toUpperCase()}`
                              : ` · QSV ${currentFidelityInspection.effectiveQSVRateControl.toUpperCase()}`
                            : ''}
                        </span>
                      </div>
        ...
        <Dialog open={trackSaveReviewOpen} onClose={() => { if (!updateSetting.isPending) { setTrackSaveReviewOpen(false); setPendingTrackProfileSave(null); } }} fullWidth maxWidth="lg">
          ...
          <DialogContent dividers>
            {trackSaveReviewPreview.isPending ? (
              <LinearProgress />
            ) : null}

            {trackSaveReviewPreview.isError ? (
              <Alert severity="error">
                Track Profile could not be resolved for review.
              </Alert>
            ) : null}

            {pendingTrackProfileSave && trackSaveReviewPreview.data ? (
              <TrackProfileSaveReview
                profile={pendingTrackProfileSave}
                conversion={trackProfileOverride(pendingTrackProfileSave)}
                source={selectedAssetSnapshot}
                asset={selectedAsset}
                resolution={trackSaveReviewPreview.data}
              />
            ) : null}
                        ...
                        <Grid container spacing={2} alignItems="flex-start">
                          ...
                          {videoWorkerValue(videoDraft, 'preferredEncoder', 'software') === 'hardware' ? <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                            <TextField
                              ...
                              onChange={(event) => selectHardwareEncoder(event.target.value)}
                          ...
                          {videoWorkerValue(videoDraft, 'preferredEncoder', 'software') === 'hardware' && hardwareUsesGlobalQuality(selectedHardwareEncoder) ? <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                            <TextField
                              ...
                              helperText={qsvAssetQualitySummary(videoDraft.workerConfig ?? {}) ?? hardwareQualityHelper(videoDraft.qualityValue)}
                              size="small"
                              fullWidth
                            />
                          </Grid> : null}
                              ...
                              return <Box key={stream.index} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 1.5 }}>
                                <Stack spacing={1.25}>
                                  ...
                                  {subtitleActionForStream(stream.index) === 'extract' || subtitleActionForStream(stream.index) === 'keep_and_extract' ? <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                                    ...
                                    <FormControlLabel control={<Checkbox checked={subtitleFormatsForStream(stream.index).includes('srt')} onChange={(event) => updateSubtitleFormatsForStream(stream.index, 'srt', event.target.checked)} />} label={isBitmapSubtitleStream(stream) ? 'SRT (OCR)' : 'SRT (Converted)'} />
                                    <FormControlLabel control={<Checkbox checked={subtitleFormatsForStream(stream.index).includes('ass')} onChange={(event) => updateSubtitleFormatsForStream(stream.index, 'ass', event.target.checked)} />} label={isBitmapSubtitleStream(stream) ? 'ASS (OCR text)' : 'ASS (Converted)'} />
                                  </Stack> : null}
                                  {isBitmapSubtitleStream(stream) && subtitleFormatsForStream(stream.index).some((format) => format === 'srt' || format === 'ass') && (subtitleActionForStream(stream.index) === 'extract' || subtitleActionForStream(stream.index) === 'keep_and_extract') ? <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                      ...
                      <Box sx={{ border: 1, borderColor: 'divider', borderRadius: 1, p: 2 }}>
                        <Stack spacing={2}>
                          ...
                          <Grid container spacing={1.5}>
                            ...
                            <Grid size={{ xs: 12, md: 4 }}><TextField select fullWidth label="ASS/SSA font attachments" value={trackDraft.fontAttachmentExportPolicy ?? 'none'} onChange={(event) => setTrackDraft((current) => migrateTrackDisposition(current, { fontAttachmentExportPolicy: event.target.value as TrackProfile['fontAttachmentExportPolicy'] }))} helperText="Export source font attachments when original ASS/SSA subtitle sidecars are extracted. This helps preserve subtitle styling on systems where the source fonts are not installed."><MenuItem value="none">Do not export</MenuItem><MenuItem value="all">Export all font attachments</MenuItem></TextField></Grid>
...
function TrackProfileSaveReview({ profile, conversion, source, asset, resolution }: { profile: TrackProfile; conversion: AssetConversionOverrideState; source?: ScanResult; asset: Asset | null; resolution: TrackProfileResolutionPreviewResponse; }) {
  ...
  const resolvedSubtitles = plan.subtitleStreams.map((resolved) => ({
    ...
    stream: source?.subtitleStreams?.find(
      (stream) => stream.index === resolved.streamIndex,
    ),
  }));

  const embeddedSubtitles = resolvedSubtitles.filter(
    ({ resolved }) =>
      resolved.action === 'keep' ||
      resolved.action === 'keep_and_extract',
  );

  const extractedSubtitles = resolvedSubtitles.filter(
    ({ resolved }) =>
      resolved.action === 'extract' ||
      resolved.action === 'keep_and_extract',
  );

  const removedSubtitles = resolvedSubtitles.filter(
  ...
  const subtitleActions = extractedSubtitles.map(
    ({ resolved, stream }) => {
      const label = stream
        ? streamLabel(stream)
        : `#${resolved.streamIndex}`;
  ...
  const subtitleExports = plan.sidecarOutputs.map((sidecar) => {
    const stream = source?.subtitleStreams?.find(
      (candidate) => candidate.index === sidecar.streamIndex,
    );
  ...
  'Subtitle exports',
    ...
    subtitleExports.length
      ? subtitleExports.join(' | ')
      : 'None',

    'Canonical resolved sidecar outputs',
  ],
...
function buildCombinedLabFFmpegCommand(input: CombinedLabCommandInput) {
  ...
  const preferredEncoder = videoWorkerValue(
    input.video,
    'preferredEncoder',
    'software',
  );

  const configuredEncoder = videoWorkerValue(
    input.video,
    'videoEncoder',
    'auto',
  );
  ...
  args.push(
    ...
    '-i',
    shellQuote(input.assetPath),
    '-t',
    String(Math.max(1, Math.min(60, input.seconds || 20))),
  );
...
function combinedVideoCommandArgs(profile: ProfileInput, scan: ScanResult) {
  ...
  if (encoder === 'hevc_qsv') {
    ...
    const qsvTenBit =
      configuredPixFmt === 'p010' ||
      configuredPixFmt === 'p010le' ||
      configuredPixFmt === 'yuv420p10' ||
      configuredPixFmt === 'yuv420p10le' ||
      configuredPixFmt.includes('10');

    const pixFmt = qsvTenBit ? 'p010le' : 'nv12';
    ...
    const qsvGlobalQuality = numberWorkerValue(
      profile,
      'globalQuality',
      qsvQualityRangeForCrf(profile.qualityValue).recommended,
    );

    if (qsvRateControl === 'vbr' || qsvRateControl === 'cbr') {
      const targetBitrate = numberWorkerValue(
        profile,
        'qsvTargetBitrate',
        0,
      );
      ...
      if (targetBitrate > 0) {
        ...
        args.push('-global_quality', String(qsvGlobalQuality));
      }

    } else {
      args.push('-global_quality', String(qsvGlobalQuality));

      if (qsvRateControl === 'cqp') {
        args.push('-flags', '+qscale');
      }
    }

    const qsvLookAheadDepth = numberWorkerValue(
      profile,
      'qsvLookAheadDepth',
      40,
    );
    ...
    if (qsvRateControl === 'la_icq') {
      ...
      if (qsvLookAheadDepth > 0) {
        args.push(
          '-look_ahead_depth',
          String(Math.min(100, Math.max(10, qsvLookAheadDepth))),
        );
      }
    }

    if (qsvTenBit) {
      const qsvExtendedBRC = videoWorkerBool(profile, 'qsvExtendedBRC');

      if (qsvExtendedBRC) {
        args.push('-extbrc', '1');

        if (qsvRateControl !== 'la_icq' && qsvLookAheadDepth > 0) {
          args.push(
            '-look_ahead_depth',
            String(Math.min(100, Math.max(10, qsvLookAheadDepth))),
          );
